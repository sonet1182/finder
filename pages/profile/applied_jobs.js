...
function AppliedJobs() {
  ...
  const fetchJobsHandler = async (pageNumber = 1, qty = perPage) => {
    ...
    axios.get("/sanctum/csrf-cookie").then((res) => {
      axios
        .get(`api/tutor/apply_tuition_list/${qty}?page=${pageNumber}`)
        .then((response) => {
    ...
    <>
      ...
      <div className="">
        <div className="bg-info mt-3 py-2 pl-2">
          <h5 className="text-title text-white px-4">Your Applied Jobs</h5>
        </div>
        ...
        <div className="content-page">
          ...
          <div className="row display-list">
            {!loading
              ? jobs?.data?.map((job, i) => <><div className="col-md-6"><JobCard key={i} job={job} /></div></>)
...
AppliedJobs.Layout = PrivateLayout;
