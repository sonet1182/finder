...
export default function CancelledJobs() {
  ...
  const fetchJobsHandler = async (pageNumber = 1, qty = perPage) => {
    ...
    axios.get("/sanctum/csrf-cookie").then((res) => {
      axios
        .get(`api/tutor/cancelled_tuition_list/${qty}?page=${pageNumber}`)
        .then((response) => {
    ...
    <>
      ...
      <div className="">
        <div className="bg-info mt-3 py-2 pl-2">
          <h5 className="text-title text-white px-4">Your Cancelled Jobs</h5>
        </div>
...
CancelledJobs.Layout = PrivateLayout;
