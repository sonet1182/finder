...
import img from "../assets/imgs/brands/brand-1.png";
import Image from "next/image";
import { FaAngleDown, FaMapMarkedAlt, FaMapMarkerAlt } from "react-icons/fa";
...
function Jobs() {
  ...
  const fetchJobsHandler = async (pageNumber = 1) => {
    setLoading(true);
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    ...
    const response = await publicApi.get(
      `api/tuition-list/10?page=${pageNumber}`
    );
