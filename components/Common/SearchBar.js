...
function SearchBar({ districts }) {
  ...
  const searchSubmitHandler = (e) => {
    e.preventDefault();
    // alert(place+" "+category+" "+searchContent)

    if(category && place && searchContent){
      router.push({
        pathname: `/jobs/`,
        query:  {search: searchContent, category: category, place: place},
    } ) 
      setSearchContent("")
      setCategory("")
      setPlace("")
          ...
          >
            <option value="0">Select District</option>
            {
              districts.map((district,index) =>{
                return (
                  <option value={district.id} key={index}>{district.districtName}</option>
                )
              })
            }
          </select>
